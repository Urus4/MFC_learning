...
public:
	...
	bool LoadImage(const char* filename)
	{
		...
		if (!strcmp(".bmp", &filename[strlen(filename)-4]))
		{
			...
			if (!fread(&fileHeader, sizeof(BITMAPFILEHEADER), 1, pFile)) //BITMAPFILEHEADER의 크기만큼 읽어옴 (14바이트)
			{
				fclose(pFile);
				return false;
			}

			if (fileHeader.bfType != 0x4D42) // 'BM' 문자 검사 , 2바이트만 읽어서 BM인지 확인
			{
				fclose(pFile);
				return false;
			}
			...
			if (!fread(&infoHeader, sizeof(BITMAPINFOHEADER), 1, pFile))	// BITMAPINFOHEADER의 크기만큼 읽어오기 (42바이트)
			{
				fclose(pFile);
				return false;
			}
